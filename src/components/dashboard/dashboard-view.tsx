  ...
  return (
    <Card className="h-full">
      ...
      <CardContent className="space-y-1 text-sm">
        ...
        <p className="text-muted-foreground">
          Gross {formatCurrency(metric.grossProfit, currency)}
        </p>
...
export function DashboardView({
  ...
  }, [customFrom, customTo, kpis.marketplaceId, rangePreset, search]);
  ...
  const onSyncNow = useCallback(async () => {
    ...
    try {
      ...
      setSyncMessage(
        `Sync complete. Orders fetched: ${payload.result?.orders?.ordersFetched ?? 0}`,
      );
  ...
  }, [kpis.marketplaceId, loadRows, router]);
        ...
        <CardContent className="space-y-4">
          {activeTab === "products" ? (
            <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">
              Products tab placeholder. Product-level rollups can be layered on top of the same
              daily summary and order-item records.
            </div>
          ) : (
            <>
                    ...
                    {isRowsLoading ? (
                      ...
                      rows.map((row) => (
                        <tr key={row.id} className="border-t align-top">
                          ...
                          <td className="px-3 py-2">
                            {formatCurrency(row.refundCost, kpis.currency)}
                          </td>
                          <td className="px-3 py-2">{formatCurrency(row.cogs, kpis.currency)}</td>
                          <td className="px-3 py-2">
                            {formatCurrency(row.amazonFees, kpis.currency)}
                          </td>
